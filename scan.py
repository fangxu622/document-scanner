...
ap.add_argument("-i", "--image", required = False,
				default=r"F:\SZU-Prj\jiaonang\2019_310_capsule_prj\document-scaner\data\39.bmp")
...
#image_tm= cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
#eqh_img_tm=cv2.equalizeHist(image_tm)
# eqh_img0=cv2.equalizeHist(image[:,:,0])
# eqh_img1=cv2.equalizeHist(image[:,:,1])
# eqh_img2=cv2.equalizeHist(image[:,:,2])
# eqh_img=cv2.merge([eqh_img0,eqh_img0,eqh_img2])
#eqh_img=cv2.merge([eqh_img_tm,eqh_img_tm,eqh_img_tm])
# convert the image to grayscale, blur it, and find edges
# in the image
meanimage = cv2.pyrMeanShiftFiltering(image, 15, 30, maxLevel=2,termcrit=(cv2.TERM_CRITERIA_MAX_ITER+cv2.TERM_CRITERIA_EPS, 5, 1))
...
#cv2.imshow("eqh_img", eqh_img_gray)
...
for c in cnts:
	...
	if len(approx)==4:#> 4 and len(approx) < 9:
		index_p.append(peri)
